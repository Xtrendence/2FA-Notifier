...
export default function ViewAccount({navigation, route}) {
  ...
  return (
    <Page
      ...
      icon="pen"
      onPressIcon={() => navigation.navigate('Edit', {account: account})}>
      <View style={styles.wrapper}>
        <CircularProgress
          ...
          maxValue={period}
          activeStrokeColor={`${
            Gradients[account.gradient][Gradients[account.gradient].length - 1]
          }`}
      ...
      <View style={styles.buttonWrapper}>
        <TouchableScale
          style={[styles.button, styles[`button${copied}`]]}
          onPress={() => copy(code)}>
          <Text style={[styles.buttonText, styles[`buttonText${copied}`]]}>
